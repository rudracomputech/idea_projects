...
class AuthController extends StateNotifier<bool> {
  ...
  Future<CommonResponse> singUp({required SingUp singUpInfo}) async {
    try {
      ...
      final accessToken = response.data['data']['access']['token']["token"];
  ...
  Future<CommonResponse> verifyOTP(
    ...
    try {
      ...
      // final String token = response.data['data']["access"]["token"]['token'];
      // ref.read(hiveServiceProvider).saveUserAuthToken(authToken: token);
